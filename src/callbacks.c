                            ...
                            pycbc_debug_info debug_info)
{
    ...
    if (check_enoent && (mres->mropts & PYCBC_MRES_F_QUIET) &&
        (err == LCB_ERR_DOCUMENT_NOT_FOUND || err == LCB_ERR_SUBDOC_PATH_NOT_FOUND)) {
        return 0;
    }
    ...
    pycbc_OperationResult *res, int cbtype, const lcb_RESPBASE *resp)
{
    ...
    if(res->rc == LCB_SUCCESS) {
        ...
        Until mlcb_resp_get_mutation_token is back in libcouchbase includes, do this: */
        Py_INCREF(Py_None);
        res->mutinfo = Py_None;
...
pycbc_callbacks_init(lcb_t instance)
{
    ...
    // Comment out until stats are back in lcb lcb_install_callback(instance, LCB_CALLBACK_STATS, stats_callback);
