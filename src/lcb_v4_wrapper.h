...
#define GENERIC_SPAN_OPERAND(SCOPE, INSTANCE, CMD, HANDLE, CONTEXT) \
    if ((CONTEXT) && (CONTEXT)->span){lcb_cmd##SCOPE##_parent_span(CMD, (CONTEXT)->span);}
...
#define VIEW_SPAN_OPERAND(SCOPE, INSTANCE, CMD, HANDLE, CONTEXT) if ((CONTEXT) && (CONTEXT)->span){lcb_cmdview_parent_span(CMD, (CONTEXT)->span);}
