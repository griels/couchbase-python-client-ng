...
{
    int rv = 0;
    PyObject *maybe_bucket = (PyObject_IsInstance(args, (PyObject *) &PyTuple_Type) && PyTuple_Size(args) > 0)
                           ? PyTuple_GetItem(args, 0) : NULL;
    if (!maybe_bucket && PyObject_IsInstance(maybe_bucket, (PyObject*)&BucketType))
    {
        PYCBC_EXC_WRAP(PYCBC_EXC_ARGUMENTS,0,"First argument must be the parent bucket")
        return -1;
    }

    rv = pycbc_collection_init_from_fn_args(self, (pycbc_Bucket*)maybe_bucket, kwargs);
