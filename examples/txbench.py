...
from txcouchbase.cluster import TxRawCluster, TxCluster
...
ap.add_argument('-U', '--connstr', default='couchbase://localhost/',
                help="Connection string")
ap.add_argument('-B', '--bucket', default='default',
                help="Bucket name")
ap.add_argument('-u', '--username', default='default', type=str)
ap.add_argument('-p', '--password', default='password', type=str)
...
ap.add_argument('--workload', type=str, default='kv', help="Workload type (kv, n1ql)")
...
class Runner(object):
    ...
    def _schedule_raw(self, *args):
        opres = self.schedule_work()
        opres.callback = self._schedule_raw
        self.opcount += 1

    def schedule_work(self):
        if options.workload == "n1ql":
            opres = self.cb.upsert_multi(self.kv, format=FMT_BYTES)
        else:
            opres = cb_cluster.query("SELECT * FROM `beer-sample` LIMIT 2")
        return opres

    def _schedule_deferred(self, *args):
        rv=self.schedule_work()
...
for _ in range(options.clients):
    cls = TxCluster if options.deferreds else TxRawCluster
    from couchbase.cluster import PasswordAuthenticator, ClusterOptions
    cb_cluster = cls.connect(options.connstr, authenticator=PasswordAuthenticator(options.username, options.password),**kwargs)
    cb = cb_cluster.bucket(options.bucket).default_collection()
    clients.append(cb)
    d = cb.on_connect()
