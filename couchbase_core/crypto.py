    ...
    def __init__(self, providername, providerfunc, initfuncs=[], providerpaths=[], keystore=None, **kwargs):
        ...
        for library in providerpaths:
            ...
            try:
                ...
                logging.error(str(e))
                raise couchbase.exceptions.NotFoundException("Couldn't load provider shared library")
        if crypto_dll:
            ...
            try:
                ...
                raise couchbase.exceptions.NotFoundException("Couldn't initialise crypto provider shared library")

            super(CTypesCryptoProvider, self).__init__(provider=provider)
            if self.keystore:
                setattr(self, 'load_key', self.key_loader)
        else:
            raise couchbase.exceptions.NotFoundException("Couldn't find provider shared library")
