...
class RowProcessor(object):
    ...
    * Fetching multiple documents for each row
        You can use the :meth:`~couchbase_v2.bucket.Bucket.get_multi`
    ...
    .. attribute:: rowclass
        ...
        * ``docid`` is the ID of the document itself (as stored by one
            of the :meth:`~couchbase_v2.bucket.Bucket.upsert` family of
            methods).
        ...
        * ``doc`` is the document itself - Only valid if ``include_docs``
            ...
            :class:`~couchbase_v2.connection.Result` object is passed.
            If ``reduce`` was set to true for the view, this will always
            be None
...
class View(object):
    def __init__(self, parent, design, view, row_processor=None,
        ...
        :type parent: :class:`~couchbase_v2.bucket.Bucket`
        ...
        :param boolean include_docs: If set, the document itself will be
            ...
            uses :meth:`~couchbase_v2.bucket.Bucket.get_multi` for each
            page (i.e. every :attr:`streaming` results).

            The :attr:`~couchbase_v2.views.params.Query.reduce`
        ...
        :param query: If set, should be a :class:`~couchbase_v2.views.params.Query` or
        ...
        :param params: Extra view options. This may be used to pass view
            arguments (as defined in :class:`~couchbase_v2.views.params.Query`)
            without explicitly constructing a
            :class:`~couchbase_v2.views.params.Query` object.
            ...
            query's :meth:`~couchbase_v2.views.params.Query.update` method
            instead.
    ...
    def query(self):
        """
        Returns the :class:`~couchbase_v2.views.params.Query` object associated
    ...
    def __iter__(self):
        ...
        :raise: :exc:`~couchbase_v2.exceptions.ViewEngineError`

            If an error was encountered while processing the view, and the
            :attr:`~couchbase_v2.views.params.Query.on_error`
