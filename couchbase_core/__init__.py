...
floatpattern=r'\d+(\.\d+)?'
regex = re.compile(r'((?P<hours>{floatpattern})hr)?((?P<minutes>{floatpattern})min)?((?P<seconds>{floatpattern})s)?((?P<milliseconds>{floatpattern})ms)?'.format(floatpattern=floatpattern))
