...
class BucketManager(object):
    ...
    to a :class:`~couchbase_v2.bucket.Bucket` object. It is normally returned via
    the :meth:`~couchbase_v2.bucket.Bucket.bucket_manager` method
    """
    ...
    def _design_poll(self, name, mode, oldres, timeout=5, use_devmode=False):
        ...
        :type oldres: :class:`~couchbase_v2.result.HttpResult`
        """
        if not timeout:
            return True
    ...
    def design_create(self, name, ddoc, use_devmode=True, syncwait=0):
        ...
        :raise: :exc:`couchbase_v2.exceptions.TimeoutError` if ``syncwait`` was
            specified and the operation could not be verified within the
            interval specified.

        :return: An :class:`~couchbase_v2.result.HttpResult` object.
    ...
    def design_get(self, name, use_devmode=True):
        ...
        :return: A :class:`~couchbase_v2.result.HttpResult` containing
            a dict representing the format of the design document

        :raise: :exc:`couchbase_v2.exceptions.HTTPError` if the design does not
            exist.
    ...
    def n1ql_index_list(self, other_buckets=False):
        ...
        :param bool other_buckets: Whether to also include indexes belonging
            ...
            object)
        :return: list[couchbase_v2._ixmgmt.Index] objects
        """
        info = N1qlIndex()
    ...
    def n1ql_index_build_deferred(self, other_buckets=False):
        ...
        :return: list[couchbase_v2._ixmgmt.Index] objects. This
