...
from couchbase_core._pyport import Protocol, unicode
...
class TermQuery(_SingleQuery):
    ...
    Useful to check what the actual content of the index is.
    It can also apply fuzziness on the term. Usual better alternative is `MatchQuery`.
...
class PhraseQuery(_SingleQuery):
    ...
    def __init__(self, *phrases, **kwargs):
        ...
        Usual better alternative is `MatchPhraseQuery`.
        """
...
class PrefixQuery(_SingleQuery):
    ...
    The usual better alternative is `MatchQuery`.
    """
...
class MatchAllQuery(Query):
    """
    Special query which matches all indexed documents
    """
    def __init__(self, **kwargs):
...
def make_search_body(index, query, params=None):
    ...
    :type params: :class:`search.Params`
