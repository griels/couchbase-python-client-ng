...
from couchbase_tests.base import RealServerTestCase, SkipTest, ClusterTestCase, CollectionTestCase
...
class IndexManagementTestCase(CollectionTestCase):
    ...
    def test_create_primary(self):
        ...
        self.assertRaises(
            E.CouchbaseError, lambda x: x.rows(), self.cluster.query(qstr))
        ...
        self.assertRaises(KeyExistsException, mgr.create_primary_index(bucket_name))

        # Drop the indexes
        mgr.drop_primary_index(bucket_name)
        mgr.drop_primary_index(bucket_name, ignore_missing=True)
        self.assertRaises(E.NotFoundError, mgr.drop_primary_index, bucket_name)

        # Test with _named_ primaries
        mgr.create_index(bucket_name, ixname, [], primary=True)
        mgr.create_index(bucket_name, ixname, [], primary=True, ignore_if_exists=True)
        self.assertRaises(E.KeyExistsException, mgr.create_index, bucket_name,
                          ixname, primary=True)
    ...
    def test_alt_indexes(self):
        ...
        mgr.create_index(bucket_name, ixname, fields=['hello'], ignore_exists=True)
        self.assertRaises(E.KeyExistsException, mgr.create_index,
                          bucket_name, ixname, fields=['hello'])

        # Drop it
        mgr.drop_index(bucket_name, ixname)
        mgr.drop_index(bucket_name, ixname, ignore_missing=True)
        self.assertRaises(E.NotFoundError, mgr.drop_index, bucket_name, ixname)
    ...
    def test_deferred(self):
        ...
        self.assertRaises(E.NotFoundError, mgr.watch_indexes, self.cluster_info.bucket_name, ['nonexist'])