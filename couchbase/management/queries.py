...
from couchbase.options import OptionBlock, forward_args, Duration
from typing import *
...
class QueryIndexManager(GenericManager):
    ...
    def drop_index(self,  # type: QueryIndexManager
        ...
        :raises: InvalidArgumentsException
        """
        info = BucketManager._mk_index_def(bucket_name, index_name, kwargs.pop('primary',False))
        IxmgmtRequest(self._admin_bucket, 'drop', info, **kwargs).execute()
    ...
    def drop_primary_index(self,  # type: QueryIndexManager
        ...
        """
        self.drop_index(bucket_name, "", primary=True, **kwargs)
...
class QueryIndex(Protocol):
    ...
    @staticmethod
    def from_n1qlindex(n1qlindex  # type: N1qlIndex
                       ):
        return QueryIndex(n1qlindex.name, bool(n1qlindex.primary), IndexType(), n1qlindex.state, n1qlindex.keyspace, [], n1qlindex.condition or "")
